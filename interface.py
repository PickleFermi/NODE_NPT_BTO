...
from torchmd.util import batch_to
...
class GNNPotentials(GeneralInteraction):
    ...
    def __init__(self, system, gnn, cutoff, ex_pairs=None, model_type="SchNet"):
        """Summary
    ...
    def forward(self, xyz, cell=None):
        ...
        if cell is None:
            cell = torch.from_numpy(np.array(self.system.cell)).float().to(self.device)
        if self.model_type == "DimeNet":
            # cell = cell.unsqueeze(0)
            # xyz = xyz.unsqueeze(0)
            result = self.gnn(xyz, cell)
            return result
        else:
...
class PairPotentials(GeneralInteraction):
    ...
    def __init__(self, system, pair_model, cutoff=2.5, index_tuple=None, ex_pairs=None, nbr_list_device=None):
        ...
        self.model = pair_model
        self.Z = system.get_atomic_numbers()
        self.cutoff = cutoff
        self.index_tuple = index_tuple
        self.ex_pairs = ex_pairs
    ...
    def forward(self, xyz):
        ...
        energy = torch.Tensor([0.0]).to(self.nbr_list_device)
        for i, atom_pair in enumerate(self.nbr_list):
            Z_i = self.Z[atom_pair[0]]
            Z_j = self.Z[atom_pair[1]]
            energy_ = self.model(pair_dis[i], (Z_i, Z_j))
            # print(f"energy={energy}, delta={energy_}, Z_i={Z_i}, Z_j={Z_j}, r={pair_dis[i]}")
            energy = energy + energy_
        return energy


class DimeNetPotentials(GeneralInteraction):
    def __init__(self, system, gnn,):
        super().__init__(system)
        self.gnn = gnn
        self.to(self.device)
...
class Stack(torch.nn.Module):
    ...
    def __init__(self, model_dict, atoms=None, mode='sum'):
        """Summary
    ...
    def forward(self, x, cell=None):
        nn_result, _, nn_stress_volume = self.models['nn'](x, cell)
        nn_stress_volume = nn_stress_volume / self.atoms.get_volume()

        prior_result, _, prior_stress_volume = self.models['pair'](x, cell)
        prior_stress_volume = prior_stress_volume / self.atoms.get_volume()
        # return prior_result, _, prior_stress_volume
        result = prior_result + nn_result
        stress_volume = prior_stress_volume + nn_stress_volume
        return result, _, stress_volume
...
class BondPotentials(torch.nn.Module):
    ...
    def __init__(self, system, top, k, ro):
        ...
        self.cell = self.cell.diag()
        self.k = k 
        self.ro = ro 
        self.top = top.to(self.device) if self.device != 'cpu' else top
