...
class Buckingham_Coulomb(torch.nn.Module):
    def __init__(self):
        super(Buckingham_Coulomb, self).__init__()
        # A(eV)
        self.A = {
            56: 1.820,
            22: 1.235,
    ...
    def Buckingham_Coulomb(self, r, A, B, C, qi, qj):
        # 计算 Buckingham 项
        f0 = 6.972 * 10e-11
        epsilon0 = 8.854 * 10-2
        buckingham_term = A * torch.exp(-r / B) - C / r**6
        # 计算库仑势项
        coulomb_term = qi * qj / r
        # 总势函数
        return coulomb_term + buckingham_term
    ...
    def forward(self, x, atom_pair):
        ...
        qi = self.q[Z_i]
        qj = self.q[Z_j]
        if atom_pair in [(56, 8), (22, 8), (8, 8), (8, 22), (8, 56)] :
            A = self.A[(Z_i, Z_j)] if (Z_i, Z_j) in self.A else self.A[(Z_j, Z_i)]
            B = self.B[(Z_i, Z_j)] if (Z_i, Z_j) in self.B else self.B[(Z_j, Z_i)]
            C = self.C[(Z_i, Z_j)] if (Z_i, Z_j) in self.C else self.C[(Z_j, Z_i)]
            energy = self.Buckingham_Coulomb(x, A, B, C, qi, qj)
            return energy
        else:
            energy = self.only_coulomb(x, qi, qj)
            return energy
